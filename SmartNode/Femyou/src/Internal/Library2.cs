...
namespace Femyou.Internal
{
  class Library2 : Library
  {
    ...
    {
      Setup(handle, currentTime, null);
    }

    public override void Setup(IntPtr handle, double currentTime, Func<bool> initialization)
    {
      fmi2SetupExperiment(
        handle,
        FMI2.fmi2Boolean.fmi2False,
      ...
      )
    {
      ...
      if (status != 0)
        throw new FmuException($"Failed to read variable(s) {string.Join(",", variables.Select(v => v.Name)).ToString()}, status: {status}");
      return values;
    }
